    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="font-display text-2xl font-bold text-white">Revenue Dashboard</h1>
        </div>
        <p className="text-gray-400 text-sm">Track MRR, subscriber growth, and revenue breakdown</p>
      </div>

      {/* Key Metrics */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { icon: DollarSign, label: "Current MRR", value: `$${currentMRR.toLocaleString()}`, sub: `+${mrrGrowth}% MoM`, color: "#00ff88" },
          { icon: Target, label: "Projected ARR", value: `$${projectedARR.toLocaleString()}`, sub: "Based on current MRR", color: "#00d4ff" },
          { icon: Users, label: "Total Subscribers", value: totalSubscribers.toLocaleString(), sub: `${subscribersByTier.length} tiers`, color: "#00d4ff" },
          { icon: TrendingUp, label: "Avg Revenue/User", value: `$${(currentMRR / totalSubscribers).toFixed(0)}`, sub: "ARPU", color: "#ffb800" },
        ].map((stat, i) => (
          <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="glass-card rounded-xl p-4">
            ...
            <div className="text-xs mt-1 font-mono" style={{ color: stat.color }}>{stat.sub}</div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="lg:col-span-3 glass-card rounded-xl p-5">
          <h3 className="font-display font-semibold text-white mb-4">MRR Growth</h3>
          <ResponsiveContainer width="100%" height={280}>
            <AreaChart data={mrrData}>
              ...
              />
              <Area type="monotone" dataKey="mrr" stroke="#00ff88" fill="url(#mrrGrad)" strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="lg:col-span-2 glass-card rounded-xl p-5">
          <h3 className="font-display font-semibold text-white mb-4">Revenue by Module</h3>
          ...
          <div className="space-y-2 mt-2">
            {revenueByModule.map((mod) => (
              <div key={mod.name} className="flex items-center justify-between text-xs">
                ...
                <span className="font-mono text-white">${mod.value.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="glass-card rounded-xl p-5">
          <h3 className="font-display font-semibold text-white mb-4">Subscribers by Tier</h3>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={subscribersByTier}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1a1a2e" />
              <XAxis dataKey="tier" tick={{ fill: "#666", fontSize: 10 }} axisLine={false} tickLine={false} />
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="glass-card rounded-xl p-5">
          <h3 className="font-display font-semibold text-white mb-4">Revenue by Tier</h3>
              ...
              return (
                <div key={tier.tier}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="font-mono text-white text-sm">${tier.revenue.toLocaleString()}</span>
                  </div>
                  <div className="flex items-center gap-2">
                  ...
                  <div className="text-gray-500 text-xs mt-1">{tier.count} subscribers</div>
                </div>
              );
            })}
          </div>
