...
import {
  Bot as BotIcon, Play, Pause, TrendingUp, TrendingDown,
  Settings, Shield, DollarSign, BarChart3
} from "lucide-react";
...
const strategies = [
  { id: "momentum", name: "Momentum", desc: "Follows strong price trends using EMA crossovers", winRate: "68%" },
  { id: "mean-reversion", name: "Mean Reversion", desc: "Buys oversold and sells overbought conditions", winRate: "62%" },
  { id: "breakout", name: "Breakout", desc: "Enters on price breakouts above resistance levels", winRate: "71%" },
];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="font-display text-2xl font-bold text-white">Trading Bot</h1>
          </div>
          <p className="text-gray-400 text-sm">Paper trading simulation with real BTC price from CoinGecko</p>
        </div>
        <button
          onClick={toggleBot}
        ...
        >
          {botActive ? <><Pause className="w-4 h-4" /> ACTIVE</> : <><Play className="w-4 h-4" /> PAUSED</>}
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card rounded-xl p-4">
          ...
          <div className="font-mono text-xl font-bold text-white">${price.toLocaleString()}</div>
          <div className={`font-mono text-xs mt-1 ${change24h >= 0 ? "text-[#00ff88]" : "text-red-400"}`}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-card rounded-xl p-4">
          ...
          <div className="font-mono text-xl font-bold text-[#00ff88]">+$1,005</div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass-card rounded-xl p-4">
          ...
          <div className="text-gray-400 text-xs font-mono mt-1">Last 7 days</div>
        </motion.div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="lg:col-span-2 glass-card rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-display font-semibold text-white">Cumulative P&L</h3>
          ...
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={pnlChartData}>
              ...
              />
              <Area type="monotone" dataKey="cumulative" stroke="#00ff88" fill="url(#pnlGrad)" strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="space-y-4">
          <div className="glass-card rounded-xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-display font-semibold text-white">Strategy</h3>
            </div>
            <div className="space-y-2">
              {strategies.map((s) => (
                <button
                  key={s.id}
                  onClick={() => { setSelectedStrategy(s.id); toast.info(`Strategy: ${s.name}`); }}
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[#00ff88] text-xs font-mono">{s.winRate}</span>
                  </div>
          ...
          <div className="glass-card rounded-xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-display font-semibold text-white text-sm">Risk Settings</h3>
            </div>
      ...
      {/* Trade History */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="glass-card rounded-xl overflow-hidden">
            ...
            <tbody>
              {tradeHistory.map((trade) => (
                <tr key={trade.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                  ...
                  <td className="py-3 px-4 text-right text-gray-300 text-sm font-mono">${trade.entry.toLocaleString()}</td>
                  <td className="py-3 px-4 text-right text-gray-300 text-sm font-mono">${trade.exit.toLocaleString()}</td>
                  <td className={`py-3 px-4 text-right text-sm font-mono font-bold ${trade.pnl >= 0 ? "text-[#00ff88]" : "text-red-400"}`}>
                    {trade.pnl >= 0 ? "+" : ""}{trade.pnl.toFixed(2)}%
                  </td>
                  <td className="py-3 px-4 text-right text-gray-500 text-xs font-mono hidden sm:table-cell">{trade.timestamp}</td>
                </tr>
              ))}
            </tbody>
          </table>
