...
const MINING_IMG =
  "https://d2xsxph8kpxj0f.cloudfront.net/310519663508251297/DLgAeusBc3GPPZNVGu2d5R/mining-landscape-Yx9JrmDxEBoBSrmwhbuUub.webp";
    ...
    <div className="space-y-6">
        ...
        <div className="relative">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="font-display text-2xl font-bold text-white">
              Mining Claim Intelligence
            </h1>
          </div>
          <p className="text-gray-400 text-sm">
            AI-powered scoring system for mining and land claims
          </p>
        </div>
      </div>
      ...
      >
        ...
        <div className="relative flex flex-col items-center justify-center h-full min-h-[160px]">
          ...
          <p className="text-white font-display font-semibold">
            Interactive Claim Map
          </p>
          <p className="text-gray-400 text-xs mt-1">
            {filtered.length} claims displayed • Click markers for details
          </p>
          <div className="flex flex-wrap gap-2 mt-3">
            {filtered.slice(0, 5).map(c => (
              <span
                key={c.id}
                className="px-2 py-1 bg-[#0a0a0f]/80 border border-white/10 rounded text-xs text-gray-300 font-mono"
              >
                <MapPin className="w-3 h-3 inline mr-1 text-[#00d4ff]" />
                {c.name}
              </span>
            ))}
            {filtered.length > 5 && (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/10 text-gray-400 text-xs font-mono uppercase">
                <th
                  className="text-left py-3 px-4 font-medium cursor-pointer hover:text-white"
                  onClick={() => handleSort("name")}
                >
                  <span className="flex items-center gap-1">
                    Claim <ArrowUpDown className="w-3 h-3" />
                  </span>
                </th>
                ...
                >
                  <span className="flex items-center gap-1">
                    Mineral <ArrowUpDown className="w-3 h-3" />
                  </span>
                </th>
                <th
                  className="text-right py-3 px-4 font-medium cursor-pointer hover:text-white hidden md:table-cell"
                  onClick={() => handleSort("acreage")}
                >
                  <span className="flex items-center gap-1 justify-end">
                    Acreage <ArrowUpDown className="w-3 h-3" />
                  </span>
                </th>
                <th
            ...
            <tbody>
              {filtered.map(claim => (
                <tr
                  key={claim.id}
                  className={`border-b border-white/5 hover:bg-white/[0.02] transition-colors ${claim.premium ? "relative" : ""}`}
                >
                  ...
                  <td className="py-3 px-4">
                    <span className="px-2 py-0.5 rounded bg-white/5 text-gray-300 text-xs font-mono">
                      {claim.mineral}
                    </span>
                  </td>
                  <td className="py-3 px-4 text-right text-gray-300 text-sm font-mono hidden md:table-cell">
                    {claim.acreage.toLocaleString()}
                  </td>
                  <td className="py-3 px-4">
      ...
      <div className="glass-card rounded-xl p-6 border-[#ffb800]/20 bg-gradient-to-r from-[#ffb800]/5 to-transparent">
        ...
        <p className="text-gray-400 text-sm mb-4">
          7 high-value claims with AI scores above 85 are available exclusively
          to Pro and Enterprise subscribers. Includes detailed geological
          reports, satellite imagery, and risk assessments.
        </p>
        <button
