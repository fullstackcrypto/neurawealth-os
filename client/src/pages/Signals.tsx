...
import { generateSignal, formatTelegramSignal, type TechnicalSignal } from "@/lib/technicalAnalysis";
import { motion } from "framer-motion";
import { Activity, RefreshCw, Send, TrendingUp, TrendingDown, Minus, Clock, AlertTriangle } from "lucide-react";
...
function SignalBadge({ signal }: { signal: "BUY" | "SELL" | "HOLD" }) {
  const config = {
    ...
    HOLD: { bg: "bg-[#ffb800]/15", text: "text-[#ffb800]", border: "border-[#ffb800]/30" },
  };
  const c = config[signal];
  return (
    <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-mono font-bold border ${c.bg} ${c.text} ${c.border}`}>
...
function CoinSignalRow({ coin, index }: { coin: CoinMarket & { techSignal: TechnicalSignal }; index: number }) {
  const handleSendTelegram = () => {
    const msg = formatTelegramSignal(coin.name, coin.symbol, coin.current_price, coin.techSignal);
    navigator.clipboard.writeText(msg).then(() => {
      toast.success("Signal copied! Paste in Telegram to share.", { duration: 3000 });
    }).catch(() => {
    ...
    >
      <td className="py-3 px-3">
        <div className="flex items-center gap-2.5">
          <span className="text-gray-500 font-mono text-xs w-5">{coin.market_cap_rank}</span>
          {coin.image ? (
          ...
          <div>
            ...
            <div className="text-gray-500 text-[10px] font-mono uppercase">{coin.symbol}</div>
          </div>
        </div>
      </td>
      ...
      <td className="py-3 px-3 text-right hidden sm:table-cell">
        <span className={`font-mono text-xs ${coin.price_change_percentage_24h >= 0 ? "text-[#00ff88]" : "text-red-400"}`}>
          {coin.price_change_percentage_24h >= 0 ? "+" : ""}{coin.price_change_percentage_24h?.toFixed(2)}%
        </span>
      </td>
      <td className="py-3 px-3 text-right hidden md:table-cell">
        <span className="font-mono text-xs text-gray-400">{coin.techSignal.rsi.toFixed(1)}</span>
      </td>
      <td className="py-3 px-3 text-right hidden md:table-cell">
        <span className={`font-mono text-xs ${
          coin.techSignal.emaCrossover === "BULLISH" ? "text-[#00ff88]" :
          coin.techSignal.emaCrossover === "BEARISH" ? "text-red-400" : "text-gray-400"
        }`}>
          {coin.techSignal.emaCrossover}
        </span>
      </td>
      <td className="py-3 px-3 text-right hidden lg:table-cell">
        <span className={`font-mono text-xs ${coin.techSignal.macd.histogram > 0 ? "text-[#00ff88]" : "text-red-400"}`}>
...
export default function Signals() {
  ...
  const coinsWithSignals = useMemo(() => {
    return coins.map((coin) => {
      const prices = coin.sparkline_in_7d?.price || Array.from({ length: 168 }, (_, i) => {
        const base = coin.current_price;
        return base * (1 + (Math.sin(i / 10) * 0.05) + (Math.random() - 0.5) * 0.02);
      });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="font-display text-2xl font-bold text-white">AI Signal Engine</h1>
          </div>
          <p className="text-gray-400 text-sm">Live crypto analysis with RSI(14), EMA(9/21), MACD — auto-refreshes every 60s</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="glass-card rounded-lg p-3 text-center">
          <div className="font-mono text-2xl font-bold text-[#00ff88]">{signalCounts.BUY}</div>
          <div className="text-gray-400 text-xs">BUY Signals</div>
        </div>
        <div className="glass-card rounded-lg p-3 text-center">
          <div className="font-mono text-2xl font-bold text-[#ffb800]">{signalCounts.HOLD}</div>
          <div className="text-gray-400 text-xs">HOLD Signals</div>
        </div>
        <div className="glass-card rounded-lg p-3 text-center">
          <div className="font-mono text-2xl font-bold text-red-400">{signalCounts.SELL}</div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/10 text-gray-400 text-xs font-mono uppercase">
                ...
                <th className="text-right py-3 px-3 font-medium hidden sm:table-cell">24h</th>
                <th className="text-right py-3 px-3 font-medium hidden md:table-cell">RSI(14)</th>
                <th className="text-right py-3 px-3 font-medium hidden md:table-cell">EMA 9/21</th>
                <th className="text-right py-3 px-3 font-medium hidden lg:table-cell">MACD</th>
                <th className="py-3 px-3 font-medium hidden sm:table-cell">AI Score</th>
